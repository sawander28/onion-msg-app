    ...
    F: Fn(&[u8]) + Send + Sync + 'static,
{
    ...
    Ok("arti-messenger proxy init".to_owned())
}
    ...
    if let Ok(mut state) = STATE.lock() {
        if let AMExState::Initialized | AMExState::Stopped = *state {
            *state = AMExState::Starting;
            ...
            info!(
                "AMEx: _configure_and_run_arti-messenger called from wrong state: {} (expected: Initialized or Stopped)",
                *state
            );
            return;
        }
    } else {
        info!("AMEx: could not lock state, aborting _configure_and_run_arti-messenger()");
        return;
    }
...
async fn _run<R: Runtime>(
    ...
    if let Ok(state) = STATE.lock() {
        if let AMExState::Starting = *state {
            ...
            let e = format!(
                "AMEx: _configure_and_run_arti-messenger called from wrong state: {} (expected: Starting)",
                *state
            );
            return Err(anyhow!(e));
        }
    } else {
        let e = "AMEx: could not lock state, aborting _configure_and_run_arti-messenger()";
        return Err(anyhow!(e));
    }
